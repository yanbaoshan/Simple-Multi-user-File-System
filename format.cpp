...
void format()
{
    ...
    //初始化空闲块链
    int last_group_block = FILEBLK - NICFREE - 1;
    block_buf[NICFREE - 1] = 0;//FILEBLK + 1 is a flag of end
    for (i = 0; i < NICFREE - 1; i++)
    {
        block_buf[NICFREE - 2 - i] = last_group_block - i - 1;
    }
    
    fs.seekp(DATASTART + BLOCKSIZE * last_group_block, std::ios::beg);
    fs.write(reinterpret_cast<char*>(block_buf), BLOCKSIZE);
    //初始化前面的组
    for (i = last_group_block - i; i > 2; i -= NICFREE)
    {
        int current_block = i;
        int next_block = i - NICFREE;
        //设置下一组的指针
        if (next_block > 2)
        {
            block_buf[NICFREE - 1] = next_block;
        }
        else
        {
            block_buf[NICFREE - 1] = 0; //最后一组的结束标志
        }
        //填充当前组的空闲块
        for (j = 0; j < NICFREE - 1; j++)
        {
            if (current_block - j - 1 > 2)
            {
                block_buf[j] = current_block - j - 1;
            }
            else
            {
                block_buf[j] = 0; //最后一组可能不足NICFREE-1块，剩余部分填0
            }
        }
        
        fs.seekp(DATASTART + BLOCKSIZE * current_block, std::ios::beg);
        fs.write(reinterpret_cast<char*>(block_buf), BLOCKSIZE);
    }

    //设置超级块中的当前栈
    int remain_blocks = (last_group_block - 1) % NICFREE; 
    int count  = 0;
    for (int k = 3; k < last_group_block; k++)
    {
        /* code */
    }
    j = (last_group_block - 1) % NICFREE  + 3; 
    for (i = j; i > 2; i--)
    {
        filsys_instance.s_free[count] = i;
        count++;
    }
    filsys_instance.s_pfree = count - 1;
    filsys_instance.s_pinode = 0;
